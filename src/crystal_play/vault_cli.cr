...
module CrystalPlay
  # VaultCli - `crystal-ansible vault <subcommand> ...`, matching real
  ...
  module VaultCli
    ...
    private def self.usage
      puts "Usage: crystal-ansible vault {encrypt|decrypt|view|encrypt_string|rekey} [options] [file ...]"
      puts ""
      puts "Examples:"
      puts "  crystal-ansible vault encrypt --vault-password-file pass.txt secrets.yml"
      puts "  crystal-ansible vault decrypt --vault-password-file pass.txt secrets.yml"
      puts "  crystal-ansible vault view --vault-password-file pass.txt secrets.yml"
      puts "  crystal-ansible vault encrypt_string --vault-password-file pass.txt --name db_password 'hunter2'"
      puts "  crystal-ansible vault rekey --vault-password-file old.txt --new-vault-password-file new.txt secrets.yml"
    end
    ...
    # Public: also used directly by crystal-play.cr's --ask-vault-pass for
    ...
    private def self.encrypt_command(args : Array(String))
      ...
      if files.empty?
        STDERR.puts "Usage: crystal-ansible vault encrypt [--vault-password-file FILE] [--output FILE] FILE ..."
        exit 1
      end
    ...
    private def self.decrypt_command(args : Array(String))
      ...
      if files.empty?
        STDERR.puts "Usage: crystal-ansible vault decrypt [--vault-password-file FILE] [--output FILE] FILE ..."
        exit 1
      end
    ...
    private def self.view_command(args : Array(String))
      ...
      if files.size != 1
        STDERR.puts "Usage: crystal-ansible vault view [--vault-password-file FILE] FILE"
        exit 1
      end
    ...
    private def self.encrypt_string_command(args : Array(String))
      ...
      unless name
        STDERR.puts "Usage: crystal-ansible vault encrypt_string [--vault-password-file FILE] --name VAR_NAME 'plaintext'"
        exit 1
      end
    ...
    private def self.rekey_command(args : Array(String))
      ...
      if files.empty?
        STDERR.puts "Usage: crystal-ansible vault rekey [--vault-password-file FILE] --new-vault-password-file FILE FILE ..."
        exit 1
      end
