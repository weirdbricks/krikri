  ...
  class TaskExecutor
    ...
    private def execute_include_vars(task : Task, host : Host) : Nil
      ...
      if task.include_vars_dir
        execute_include_vars_dir(task, vars_context, host)
        return
      end
    ...
    # (lib/ansible/plugins/action/include_vars.py): loads every vars file
    # under the directory, walking subdirectories recursively by default
    # (depth: 0 means UNLIMITED levels - depth: 1 means top-level files
    # only, each further level adds one). Files load in sorted order,
    # later files overriding earlier duplicate keys; files_matching is a
    # regex searched against the basename, ignore_files is a list of
    # regexes matched (end-anchored) against the basename, and a file
    # whose extension isn't in `extensions:` (default yaml/yml/json)
    # fails the task unless ignore_unknown_extensions: is true. Same
    # name:/register: result shapes as the file: form above.
    private def execute_include_vars_dir(task : Task, vars_context : Hash(String, JSON::Any), host : Host) : Nil
      begin
        return unless when_passes?(task, vars_context, host)
      rescue ex : WhenEvaluationError
        swallow_when_error(task, host, ex)
      ...
      unless dir = resolve_include_vars_dir_path(task, candidate)
        finish_include_vars_failure(task, host, "Unable to find file or directory named: #{candidate}")
        return
      end
      ...
      extensions = (task.include_vars_extensions || ["yaml", "yml", "json"]).map do |raw_ext|
        substitutor.substitute(raw_ext, strict: true).strip.downcase
      end
    ...
    # mirrors real Ansible's os.walk-with-sorted-entries ordering (files
    # of the current dir first, then subdirectories in sorted path
    # order). `level` starts at 1 for the top dir; a non-zero `depth`
    # stops recursion once level would exceed it (depth: 1 = top-level
    # files only). Returns the first error message encountered, or nil
    # when every candidate file was accepted (matches are appended to
    # `files`).
    private def collect_include_vars_dir_files(current : String, depth : Int32, level : Int32,
      ...
      Dir.children(current).sort.each do |entry|
        ...
        ext = File.extname(entry).lstrip('.').downcase
    ...
    # directory, the role's tasks/, the process cwd) but requires a
    # DIRECTORY at the candidate, not a file.
    private def resolve_include_vars_dir_path(task : Task, candidate : String) : String?
      return File.directory?(candidate) ? candidate : nil if candidate.starts_with?("/")
