...
module Krikri
  module JinjaFilters
    ...
    # chose 'c' style anyway) - krikri-playbook produced a `#`-commented
    ...
    Crinja.filter(:from_yaml_all) do
      ...
      values = docs.map { |doc| Krikri::VariableSubstitutor::CrinjaRenderer.json_any_to_crinja_value(JSON.parse(YAML.parse(doc).to_json)) }
    ...
    Crinja.filter({secret: ""}, :vault) do
      Crinja::Value.new(Krikri::Vault.encrypt(target.to_s, arguments["secret"].to_s))
    end
    Crinja.filter({secret: ""}, :unvault) do
      Crinja::Value.new(Krikri::Vault.decrypt(target.to_s, arguments["secret"].to_s))
    end
    ...
    Crinja.filter({key_name: "key", value_name: "value"}, :dict2items) do
      ...
      # Same bug the hand-rolled evaluator had (see Krikri.
    ...
    Crinja.test(:vault_encrypted) { Krikri::Vault.encrypted?(target.to_s) }
    ...
    # default feature libraries (which include krikri-playbook's own
    ...
    # SAME permutation within krikri-playbook itself, across runs.
    ...
    def self.crinja_lookup(env : Crinja, arguments : Crinja::Arguments, query_mode : Bool = false) : Crinja::Value
      ...
      result = case lookup_type
               ...
               when "config"
                 ...
                 values = names.map { |nval| LookupPlugins.ansible_config_value(nval) }
               ...
               when "password"
                 raw_arg = arg1.to_s
                 Crinja::Value.new(LookupPlugins.password_lookup(raw_arg, role_path))
               when "unvault"
               ...
               when "url"
                 ...
                 wantlist = arguments.kwargs["wantlist"]?.try(&.truthy?) || false
                 lines = LookupPlugins.fetch_url_lines(arg1.to_s)
                 if lines.nil?
                   Crinja::Value.new(nil)
                 elsif wantlist
               ...
               when "first_found"
                 ...
                 rendered_paths = paths.flat_map { |path_entry| LookupPlugins.resolve_first_found_roots(env.from_string(path_entry.to_s).render, role_path) }
               ...
               when "sequence"
                 Crinja::Value.new(LookupPlugins.sequence_lookup(arg1.to_s).map { |v| Crinja::Value.new(v) })
               when "csvfile"
                 Crinja::Value.new(LookupPlugins.csvfile_lookup(arg1.to_s))
               when "ini"
                 Crinja::Value.new(LookupPlugins.ini_lookup(arg1.to_s))
               else
