...
# krikri-playbook - Fast Ansible-compatible automation tool
# Main CLI entry point

# Must be required first: redefines top-level puts/print to route
# through Krikri::OutputRouting, which --forks's per-host fan-out
...
require "./src/krikri/start_at_filter"
require "./src/krikri/cli_options"
require "./src/krikri/serial_batches"
require "./src/krikri/vars_prompt"
...
if ARGV[0]? == "vault"
  Krikri::VaultCli.run(ARGV[1..])
  exit
end

# `krikri-playbook __async_run <module> <config_path> <status_path>` is an
...
if ARGV[0]? == "__async_run"
  ...
  local_host = Krikri::Host.new("localhost")
  result = Krikri::PluginManager.execute_plugin(module_name, config_json, local_host, {} of String => JSON::Any)
...
begin
  OptionParser.parse do |parser|
    ...
    parser.on("-h", "--help", "Show this help") do
      ...
      puts ""
      puts "Examples:"
      puts "  krikri-playbook playbook.yml"
      puts "  krikri-playbook --check --diff playbook.yml"
      puts "  krikri-playbook --no-batching playbook.yml"
      puts "  krikri-playbook --forks 10 playbook.yml"
      puts "  krikri-playbook --gathering smart playbook.yml"
      puts "  krikri-playbook -i inventory.ini playbook.yml"
      puts "  krikri-playbook --tags deploy playbook.yml"
      puts "  krikri-playbook --vault-password-file pass.txt playbook.yml"
      puts ""
      puts "Vault:"
      puts "  krikri-playbook vault encrypt|decrypt|view|encrypt_string|rekey ..."
      exit
    end
  ...
  puts ""
  puts "Run 'krikri-playbook --help' for usage information"
  exit 1
rescue ex : Exception
  puts "Error: #{ex.message}".colorize(:red)
  exit 1
...
if playbook_file.empty?
  ...
  puts "Usage: krikri-playbook [options] playbook.yml"
  puts "Try 'krikri-playbook --help' for more information"
  exit 1
end
...
playbook = nil
begin
  playbook = Krikri::TimingProfile.measure("parse.playbook") { Krikri::PlaybookParser.parse(playbook_file) }
...
if list_hosts_only
  ...
  Krikri::TaskLister.list_hosts(playbook || raise "BUG: playbook not parsed") do |play|
...
playbook.plays.each_with_index do |play, _play_index|
  ...
  unless play.vars_prompt.empty?
    Krikri::VarsPrompt.resolve(play.vars_prompt).each do |name, value|
  ...
  if (start_at = start_at_task) && start_at_pending
    tasks_to_run, start_at_found = Krikri::StartAtFilter.apply(tasks_to_run, start_at)
...
if show_custom_stats && !Krikri::CustomStats.empty?
  puts "CUSTOM STATS: ".colorize(:cyan).bold
  unless Krikri::CustomStats.global.empty?
    puts "\t#{Krikri::CustomStats.global.to_json}"
  end
  Krikri::CustomStats.per_host_data.each do |host_name, host_stats|
