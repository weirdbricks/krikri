...
**`lablabs.rke2` FIXED (0.9.786) - see `KNOWN_MISSING.md`'s round 43000-43201 narrative for the full story**: argument-spec `default:` expressions are now templated strictly (real Ansible templates the ENTIRE spec, failing even with the option provided); rke2 now fails at the validate_argument_spec task with the identical "object of type 'dict' has no attribute 'masters'" message, byte-identical recaps cold and warm (round 43200).
...
**`rvm.ruby` RE-VERIFIED (0.9.786), no krikri gap - see `KNOWN_MISSING.md`'s round 43000-43201 narrative**: with working egress and the role's `rvm1_user: ubuntu` default worked around (`-e rvm1_user=root`), both engines fail the identical dead-keyserver loop (sks pool decommissioned, mit/pgp.com dead), both run the rvm.io fallback; the original "krikri gpg-keyserver gap" was an artifact (real Ansible never reached the gpg tasks in round 159). Did expose one small real gap, fixed in the same version: `command:` didn't expand a leading `~` in the executable path (real Ansible's run_command expand_user).
...
**FIXED (0.9.786) - see `KNOWN_MISSING.md`'s round 43000-43201 narrative**: strict-undefined loop-item rendering with the dict-miss message refinement ("object of type 'dict' has no attribute 'data-dir'"). Round 43000: byte-identical recaps both engines, cold AND warm (real Ansible fails cold and warm identically - a genuine upstream-role bug, `etcd_config['data-dir']` on a dict missing that key).
