...
describe Krikri::PluginHelpers::RhsmRepository do
  ...
  describe ".plan" do
    ...
    it "purge disables enabled repos outside the requested list" do
      ...
      plan.enable.empty?.should be_true
      plan.updated.find! { |repo| repo.id == "rhel-7-server-optional-rpms" }.enabled?.should be_false
      plan.updated.find! { |repo| repo.id == "rhel-7-server-rpms" }.enabled?.should be_true
    end
