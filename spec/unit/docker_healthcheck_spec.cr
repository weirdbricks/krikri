...
# doc comment.
describe CrystalPlay::PluginHelpers::DockerHealthcheck do
  describe ".duration_to_ns" do
    it "parses a plain seconds duration" do
      CrystalPlay::PluginHelpers::DockerHealthcheck.duration_to_ns("10s").should eq(10_000_000_000_i64)
    end

    it "parses a combined minutes+seconds duration" do
      CrystalPlay::PluginHelpers::DockerHealthcheck.duration_to_ns("1m30s").should eq(90_000_000_000_i64)
    end

    it "parses a full hours+minutes+seconds duration" do
      CrystalPlay::PluginHelpers::DockerHealthcheck.duration_to_ns("5h34m56s").should eq(20_096_000_000_000_i64)
    end

    it "parses milliseconds/microseconds" do
      CrystalPlay::PluginHelpers::DockerHealthcheck.duration_to_ns("500ms").should eq(500_000_000_i64)
      CrystalPlay::PluginHelpers::DockerHealthcheck.duration_to_ns("500us").should eq(500_000_i64)
    end

    it "raises on an unparseable duration" do
      expect_raises(Exception, "Invalid time duration") do
        CrystalPlay::PluginHelpers::DockerHealthcheck.duration_to_ns("not-a-duration")
      end
    end
  end
  ...
  describe ".normalize_test" do
    it "passes a list test straight through as strings" do
      ...
      CrystalPlay::PluginHelpers::DockerHealthcheck.normalize_test(test).should eq(["CMD", "curl", "--fail", "http://localhost"])
    end
    ...
    it "wraps a plain string test as CMD-SHELL (matches real Ansible's normalize_healthcheck_test)" do
      ...
      CrystalPlay::PluginHelpers::DockerHealthcheck.normalize_test(test).should eq(["CMD-SHELL", "curl --fail http://localhost"])
    end
  end
  ...
  describe ".parse" do
    ...
    it "returns nil when test: is missing entirely (matches real Ansible's parse_healthcheck - no override at all)" do
      CrystalPlay::PluginHelpers::DockerHealthcheck.parse(%({"interval": "30s"})).should be_nil
    end
  end
end
