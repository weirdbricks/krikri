...
describe Krikri::ConditionalEvaluator do
  ...
  it "treats `is in` as the containment operator" do
    Krikri::ConditionalEvaluator.evaluate("item is in ignore", vars).should be_true
    Krikri::ConditionalEvaluator.evaluate("other is in ignore", vars).should be_false
  end

  it "treats `is not in` as the negated containment operator" do
    Krikri::ConditionalEvaluator.evaluate("item is not in ignore", vars).should be_false
    Krikri::ConditionalEvaluator.evaluate("other is not in ignore", vars).should be_true
  end

  it "still handles the bare operator spellings unchanged" do
    Krikri::ConditionalEvaluator.evaluate("item in ignore", vars).should be_true
    Krikri::ConditionalEvaluator.evaluate("other not in ignore", vars).should be_true
  end
  ...
  it "does not disturb an unrelated `is not <test>` condition" do
    ...
    Krikri::ConditionalEvaluator.evaluate("missing is not defined", vars).should be_true
    Krikri::ConditionalEvaluator.evaluate("item is defined", vars).should be_true
  end
end
