...
| prometheus.prometheus.node_exporter | ✅ Clean (re-verified live 2026-08-13, round 22, after round 21's 16 engine bugs plus 6 more found closing out the Crinja convergence work: `namespace()` + `{% set ns.attr = ... %}` (round 21's own blocker), a ternary/`{% for x in y if COND %}` parsing collision (the inline-ternary patch swallowed the for-loop's own `if` filter clause), `.startswith()`/`.endswith()` Python string methods missing, role defaults not crossing an `include_role:` boundary (a real executor bug, not Crinja - `_common`-style shared roles couldn't see the calling role's own defaults), `{% set a, b = expr %}` tuple-target assignment unsupported, postfix `[index]`/`.attr` after a parenthesized expression unsupported, and `not X is Y` parsed as `(not X) is Y` instead of `not (X is Y)`. Idempotent (`changed=0` on rerun), `node_exporter` service verified live via `systemctl`/`curl :9100/metrics`. See `KNOWN_MISSING.md`/`CRINJA.md` for full detail. |
